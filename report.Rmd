...
```{r summary-stats, warning=FALSE, message=FALSE, fig.show='hold'}
...
A series of matrix plots, Figure \@ref(fig:pairs-plot), do not appear to suggest that there are any non-linear relationships in the data. However, we do see a positive linear relationship between "free sulfur dioxide" and "total sulfur dioxide." The correlation coefficient for the two variables is 0.72 and a correlation plot, Figure \@ref(fig:cor-heatmap), is shown below. It is not surprising that the variables are related, since total sulfur dioxide is defined as the sum of free and bound sulfur dioxide. There is also slight collinearity between residual sugar and density, which has a correlation coefficient of 0.55. Third, we notice a high negative correlation between alcohol and density of -0.69. It is intuitive that increasing the alcohol percentage would decrease the density, as alcohol is less dense than water and juice.

```{r cor-heatmap, fig.cap="Untransformed Variable Correlations", fig.show='hold', fig.height=5, fig.width=5}
wine_cor <-
  wine %>%
...
```{r pairs-plot, message=FALSE, fig.cap="Pairs Plot of Select Variables (Log-Transformed)", fig.show='hold'}
...
```{python train-models, eval=FALSE}
...
Unfortunately, these "superwines" fail to achieve 10 ratings across all models, which is to be expected, since they were optimized for these particular models. However, they do indicate certain trends. As seen in Table \@ref(tab:simwine-preds-tab), they tend to have higher levels of free sulfur dioxide but lower levers of total sulfur dioxide than the average wine in the data, and have increased levels of chlorides and residual sugar.
...
Namely, a wine's color, fixed acidity, citric acid, residual sugar, and chlorides are all largely determined by grape characteristics rather than effects of the fermentation process. Limiting ourselves to these components alone, our best model achieves a mean squared error of `r sh_rf_mse$mean`, (C.I. of [`r sh_rf_mse$lb`, `r sh_rf_mse$ub`]), which is worse than the best model trained on all variables (`r all_rf_mse$mean`, C.I. of [`r all_rf_mse$lb`, `r all_rf_mse$ub`]), but nonetheless represents an improvement over the naive baseline, performing on par with the linear model that uses all data. Obtaining such an early estimate could help guide the manufacturing process, alerting winemakers to potential problems with a batch.
...
## Remaining Ridge Plots {#appendix-ridge}

Here we include ridge plots of the remaining variables for completeness.

```{r extra-ridge-plots, message=FALSE, warning=FALSE, message=FALSE, fig.cap="Density Plots of Select Variables by Quality", fig.show='hold'}
# make ridgeplots
ridgeplotlist_extra <- lapply(c("fixed acidity", "citric acid", "residual sugar", "chlorides", 
"free sulfur dioxide", "density", "pH"), ridgeplot)
ridgeplots_grid_extra <- ggpubr::ggarrange(plotlist = ridgeplotlist_extra, ncol = 2, nrow = 2)
ridgeplots_grid_extra[[1]]
ridgeplots_grid_extra[[2]]
```
...
```{r plot-clustering, warning=FALSE, fig.cap="Cluster Plots"}
...
### Setup
Knitr Setup
```{r code_setup, ref.label='setup', eval = FALSE, echo=TRUE}

```

Load R packages
```{r code_load_pkgs, ref.label='load_pkgs', eval = FALSE, echo=TRUE}

```

Load data
```{r code_load_data, ref.label='load_data', eval = FALSE, echo=TRUE}

```

In order to avoid fighting with factors, we make a function to move from hard-coded numbers to colors.
```{r code_helper-functions, ref.label='helper-functions', eval = FALSE, echo=TRUE}

```

### Data exploration
Create table of summary statistics.
```{r code_summary-stats, ref.label='summary-stats', eval = FALSE, echo=TRUE}

```

Create histograms.
```{r code_histograms, ref.label='histograms', eval = FALSE, echo=TRUE}

```

Create boxplots to compare log transformed data with original concentration values.
```{r code_boxplots, ref.label='boxplots', eval = FALSE, echo=TRUE}

```

Plot a correlation heatmap of the variables.
```{r code_cor-heatmap, ref.label='cor-heatmap', eval = FALSE, echo=TRUE}

```

Log-transform the data.
```{r code_variable_transformations, ref.label='variable_transformations', eval = FALSE, echo=TRUE}

```

Make a pairs-plot of the log-transformed data.
```{r code_pairs-plot, ref.label='pairs-plot', eval = FALSE, echo=TRUE}

```

Generate ridge plots of a few select variables.
```{r code_ridge-plots, ref.label='ridge-plots', eval = FALSE, echo=TRUE}

```

Save the data for training.
```{r code_save-data, ref.label='save-data', eval = FALSE, echo=TRUE}
...
```{python code_imports, ref.label='imports', eval = FALSE, echo=TRUE}

```

Setup functions to train models.
```{python code_train-functions, ref.label='train-functions', eval = FALSE, echo=TRUE}

```

Setup functions to evaluate models.
```{python code_eval-functions, ref.label='eval-functions', eval = FALSE, echo=TRUE}

```

Load the data and split into train and test sets.
```{python code_load-data, ref.label='load-data', eval = FALSE, echo=TRUE}

```

Train the models.
```{python code_train-models, ref.label='train-models', eval = FALSE, echo=TRUE}

```

Calculating errors on bootstrapped test data.
```{python code_eval-models, ref.label='eval-models', eval = FALSE, echo=TRUE}

```

Save the random forest feature importances.
```{python code_rf-feature-importances, ref.label='rf-feature-importances', eval = FALSE, echo=TRUE}
...
```{r code_make_benchmark_outputs, ref.label='make_benchmark_outputs', eval = FALSE, echo=TRUE}

```

Report on model performance.
```{r code_regtask-results-tab, ref.label='regtask-results-tab', eval = FALSE, echo=TRUE}

```

```{r code_regtask-results-fig, ref.label='regtask-results-fig', eval = FALSE, echo=TRUE}

```

```{r code_regtask-results-fig2, ref.label='regtask-results-fig2', eval = FALSE, echo=TRUE}

```

```{r code_clftask-results-tab, ref.label='clftask-results-tab', eval = FALSE, echo=TRUE}

```

```{r code_clftask-results-fig, ref.label='clftask-results-fig', eval = FALSE, echo=TRUE}

```

```{r code_clftask-results-fig2, ref.label='clftask-results-fig2', eval = FALSE, echo=TRUE}

```

Analyze the RF classification error (first running in python, and analyzing in R).
```{python code_error-analyze, ref.label='error-analyze', eval = FALSE, echo=TRUE}

```

Make a confusion matrix.
```{r code_confusion-matrix, ref.label='confusion-matrix', eval = FALSE, echo=TRUE}
...
```{r code_calculate_linear_reg, ref.label='calculate_linear_reg', eval = FALSE, echo=TRUE}

```

Make the table of linear regression coefficients.
```{r code_linear-reg-tab, ref.label='linear-reg-tab', eval = FALSE, echo=TRUE}
...
```{r code_rf-impvar-results-tab, ref.label='rf-impvar-results-tab', eval = FALSE, echo=TRUE}

```

Make function to get statistics of RF feature importances.
```{r code_rf_impvar_get_stat, ref.label='rf_impvar_get_stat', eval = FALSE, echo=TRUE}

```

### "Superwine" and "Superhuman"

Moving back to python, we use our models to evaluate a simulated wine.
```{python code_superwine, ref.label='superwine', eval = FALSE, echo=TRUE}

```

Load in these results to R.
```{r code_superwine-eval, ref.label='superwine-eval', eval = FALSE, echo=TRUE}

```

Make a table of the superwine features.
```{r code_simwine-preds-tab, ref.label='simwine-preds-tab', eval = FALSE, echo=TRUE}

```

Plot predictions of models on the superwines.
```{r code_simwine-preds-fig1, ref.label='simwine-preds-fig1', eval = FALSE, echo=TRUE}

```

Pull out the results of the early-detection model.
```{r code_superhuman-results, ref.label='superhuman-results', eval = FALSE, echo=TRUE}
...
```{r code_mclust, ref.label='mclust', eval = FALSE, echo=TRUE}

```

```{r code_plot-clustering, ref.label='plot-clustering', eval = FALSE, echo=TRUE}
