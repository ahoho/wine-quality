...
As part of the initial scoping out of this project, we have been employed by Delicious AI to investigate the possibility of using machine learning techniques to automatically evaluate a range of high quality wines in a reliable, accurate, efficient, and scalable manner. We determine which features are associated with highly rated wines and explore whether it will be possible for an algorithm to provide the recipe for a wine better than any seen before. Finally, we develop an early detection AI sommelier that can predict wine quality from grapes that are still on the vine, guiding the wine-making process from beginning to end.
...
- Our predictive benchmarking experiments were first carried out with wine quality treated as a continuous variable (univariate regression) and then repeated with wine quality treated as a categorical variable (deterministic classification)
...
- The SVM has previously been identified as the best performing model for classifying wine by quality
- linear regression model, Random Forests are relatively easy to understand and quick to implement
- Random forests: able to deal with a minimally pre-processed input data, can perform automatic feature selection, and provide information on the importance of model features

Neural networks
- able to create and combine new features independently
- perform well on classification tasks
- harder to interpret the relationship between features and classification decisions
- able to work with unlabelled data

- The random forest algorithm is the best performer on both metrics under consideration for the regression task
- mse: 0.30, mae: 0.39

- Dummy regressor
- mse: 0.69, mae: 0.65

- On the MSE metric, the SVM is the next best performer, followed by the linear model, the MLP, and finally the dummy regressor
- wine colour alone is not a strong predictor of quality, error increases for all models
- without wine color, error had more of an adverse effect on deterministic classification than regression
...
- alcohol is most important
- followed by volatile acidity i.e. mass concentration of acetic acid and density
- fixed acidity and color are least important

- By creating mixtures of chemicals (and color) -- limited only to the ranges observed in the test data
- we attempt to simulate a wine that will achieve a perfect "10" rating by our models' estimates

- We use a support vector machine (SVM) and linear regression model to produce two possible "super-wines", since they are both capable of predicting values beyond the range of data used in training

- linear regression
- maximize the values of each feature known to correspond to high quality wines
- minimize those associated with low-quality wines

SVM
- randomly sample values for each predictor (within the range of the training data) and feed the artificial wine into the SVM, stopping when we achieve a rating above 10

- fail to achieve 10 ratings across all models, which is to be expected, since they were optimized for these particular models

- higher levels of free sulfur dioxade but lower levels of total sulfur dioxide than the average wine in the data
- increased levels of chlorides and residual sugar

- a wine's color, fixed acidity, citric acid, residual sugar, and chlorides are all largely determined by grape characteristics rather than effects of the fermentation process. Limiting ourselves to these components alone, our best model acheives a mean squared error of 0.49
- best on all data: 0.30
- outperforms linear model on all data: 0.55
- help guide the manufacturing process, alerting winemakers to potential problems with a batch

## Limitations

- without more information we are unable to determine with certainty if Delicious AI can outperform humans' predictive performance on any given metric
- some of these characteristics are present early in the winemaking process, perhaps well before an expert could make a judgement of quality (although here too we would need more information to make such a determination)
