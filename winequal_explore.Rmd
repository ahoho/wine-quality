...
def config_reg_cv(learner, params=None, scorer=reg_score,
                  n_iter=50, folds=5, njobs=5, **kwargs):
  """
  Pre-configure the RandomizedSearchCV for a classifier
  """
  if params and not isinstance(learner, LinearRegression):
...
def config_clf_cv(learner, params=None, scorer=f1_score,
                  n_iter=50, folds=5, njobs=5, **kwargs):
  """
  Pre-configure the RandomizedSearchCV for a classifier
  """
...
params = [
  ...
  ('rf', {
      ...
      'max_depth': [None, scipy.stats.randint(low=2, high=features//2)]
    }
  ),
  ('mlp', {
...
classifiers = {
  name: config_clf_cv(l, p) 
  for (name, learner), params in zip(classifiers, params)
}
```
